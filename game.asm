...
################################################################
	...
	jal readfile
	lw $s3, 4($sp)		# save return val (#chars) to reg
...
################################################################
	...
	# color_bitmap function:
		...
		# return val: none
		# result: 
			# bitmap data will be stored at 0($gp)
			# player pos + exit pos are saved in $s regs
...
mainLoop:
	...
	syscall # $v0 contains the char codepoint
    ...
    j mainLoop            # unknown input? back to loop
...
# result: if new pos is valid, move player + update pos regs ($s1, $s2)

addi $sp, $sp, -8 # reserve space on stack for fp and ra
sw $fp, 0($sp) # push fp
sw $ra, 4($sp) # push ra
...
# Check if new pos is valid:
	...
	j valid_pos # if all the tests passed, then the new position is valid
...
j change_pos
# args: old pos, new pos
# return val: none
# result: 
	# clear the pixel at old pos
	# put the player's pixel at new pos

# (note: at this point, $ra contains the address under the 'jal move_player' that was called)
...
j readbuffer
    ...
    # RESULT: branch to appropriate 'letter case'
